...
impl Voting {
    ...
    pub fn get_account_gifts(&self, account_id: String) -> Vec<Gift> {
        match self.gifts.get(&account_id) {
            Some(v) => v.values().collect(),
            None => Vec::new(),
        }
    }
    ...
    pub fn contribute_to_gift(&mut self, account_name: String, gift_url: String)  {
        let amount_given = env::attached_deposit() as u128 / 10u128.pow(24);
...
mod tests {
    ...
    fn add_gift() {
        ...
        assert_eq!(
            contract
                .gifts
                .get(&"signer.testnet".to_string())
                .unwrap()
                .get(&gift1_url.to_string())
                .unwrap()
                .get_url(),
            gift1_url
        );
